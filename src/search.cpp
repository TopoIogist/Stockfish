  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
            || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY)
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos, depth >= ObliviousDepthLimit)
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_NONE)
            ss->staticEval = eval = evaluate(pos, depth >= ObliviousDepthLimit);
...
moves_loop: // When in check, search starts from here
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (   pos.is_draw(ss->ply)
        || ss->ply >= MAX_PLY)
        return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos, depth >= ObliviousDepthLimit) : VALUE_DRAW;
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos, depth >= ObliviousDepthLimit);
            ...
            (ss-1)->currentMove != MOVE_NULL ? evaluate(pos, depth >= ObliviousDepthLimit)
